...
This module exposes only local validation behavior over the three declared
public contract paths. It performs no network, credential, authorization,
tool, product, brokerage or real-world execution.
"""
...
from ._validators import agent_authority, eav, point_in_time
...
class ValidationResult:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "claim_boundary": "SDK RELEASE CANDIDATE VALIDATION DOES NOT ESTABLISH PRODUCTION AUTHORITY OR PRODUCT IMPLEMENTATION",
        }
...
_CONTRACTS: dict[str, _ContractDescriptor] = {
    ...
    "AX-PUB-SPEC-003": _ContractDescriptor(
        ...
        validator=point_in_time.validate_envelope,
    ),
    "AX-PUB-SPEC-004": _ContractDescriptor(
        ...
        validator=agent_authority.validate_envelope,
    ),
}
...
def _unsupported_result(
    ...
    return ValidationResult(
        ...
        valid=False,
        findings=(Finding(category=category, source_code=code, path="$", message=message),),
    )


def supported_contracts() -> tuple[Mapping[str, str], ...]:
    """Return the bounded release-candidate contract inventory."""
    return tuple(
        {
...
def validate(contract_id: str, payload: Any, *, version: str = "1.0") -> ValidationResult:
    """Validate one declared public contract locally and fail closed.

    Findings are returned as data. This function performs no network, tool,
    credential, product, brokerage or real-world execution.
    """
    ...
    if descriptor is None:
        return _unsupported_result(
            ...
            "AX-SDK-RC-CONTRACT-UNSUPPORTED",
            "contract is outside the declared Gate-05 release-candidate surface",
        )
    if version != descriptor.version:
        return _unsupported_result(
            ...
            "AX-SDK-RC-VERSION-UNSUPPORTED",
            f"supported release-candidate contract version is {descriptor.version}",
        )

    source_findings = descriptor.validator(payload)
    findings = tuple(
        Finding(
